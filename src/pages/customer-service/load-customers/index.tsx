...
import { Add as IoIosAdd, Settings, Print, Download as DownloadIcon, FilterList as FilterListIcon } from '@mui/icons-material';
...
const ViewCustomers: React.FC = () => {
  ...
  const { data: customer, isLoading, isError:_customerError } = useQuery({
          ...
          <TableRow key={customer._id} hover sx={{ '&:last-child td': { border: 0 } }}>
            ...
            <TableCell align="center" sx={{ py: 0.5 }}>
              <VerticalMenu actions={[
                hasAccess(["customers"], "update", currentUser) ? { label: 'Edit', icon: "edit", onClick: () => handleEditClick(customer) } : null,
                hasAccess(["customers"], "delete", currentUser) ? { label: 'Delete', icon: "delete", onClick: () => handleDeleteCustomer(customer._id || '') } : null,
                ...
                hasAccess(['customers'], "create", currentUser) ? { label: 'Report', icon: "reports", onClick: () => navigate(`${paths.customers}/report/${customer._id}`) } : null,
              ]} />
            </TableCell>
          </TableRow>
        )}
