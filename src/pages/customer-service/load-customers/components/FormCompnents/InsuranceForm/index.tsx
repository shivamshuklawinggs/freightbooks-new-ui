 ...
 const commercialExpiryDate = form.watch('commercialGeneralLiability.expiryDate');
     useEffect(() => {
        ...
        if (!isEditMode) {
          ...
          const automobileLiability= form.watch('automobileLiability');
          const cargoLiability = form.watch('cargoLiability');
          
          if (!automobileLiability?.expiryDate) {
            form.setValue('automobileLiability.expiryDate', commercialExpiryDate);
          }
          if (!automobileLiability?.issueDate) {
            form.setValue('automobileLiability.issueDate', commercialIssueDate);
          }
    
          if (!cargoLiability?.expiryDate) {
            form.setValue('cargoLiability.expiryDate', commercialExpiryDate);
          }
          if (!cargoLiability?.issueDate) {
    ...
    <Controller
      ...
      render={({ field }) => (
        <TextField size='small' {...field}
         ...
         type="number" fullWidth label="Amount" InputLabelProps={{ shrink: true }} />
      )}
    />
  </Grid>
