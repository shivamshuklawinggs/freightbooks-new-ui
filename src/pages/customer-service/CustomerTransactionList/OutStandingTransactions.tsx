...
const OutStandingTransactions: React.FC = () => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const response = await apiService.getTransactionsByCustomerId(
        id as string,
        pageParam,
        ITEMS_PER_PAGE,
        selectedYear
  ...
  const handleEdit = (invoice: ICustomerInvoicesPaymentDetails) => {
    try {
      if (invoice.transaction === "Journal Entry") {
        ...
        navigate(`${paths.recievedpayment}/${invoice._id}`);
      }
    } catch (error) {
      console.error(error)
    }
  ...
  return (
    <Box>
      <FilterTransaction years={years} selectedYear={selectedYear} setSelectedYear={setSelectedYear} totalLoaded={totalLoaded} totalRecords={totalRecords} type='customer'/>
      <PaginatedTable
  data={invoiceData}
