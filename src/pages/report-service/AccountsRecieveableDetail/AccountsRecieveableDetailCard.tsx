    ...
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: '1600px', mx: 'auto' }}>
        ...
        <Table size="small">
          <TableHead>
            <TableRow sx={{ backgroundColor: '#f8f9fa' }}>
              ...
              <TableCell sx={{ fontWeight: 700, py: 2 }}>Customer Display Name</TableCell>
              ...
              <React.Fragment key={group.bucket}>
                ...
                {group.invoices.map((invoice,index:number) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Typography sx={{cursor:"pointer"}} onClick={()=>navigate(`${paths.editinvoice}/${invoice._id}`)} variant="body2" color="primary">
                        {invoice.transactionType}
                      </Typography>
                    </TableCell>
                    <TableCell>{invoice.num || "-"}</TableCell>
                    <TableCell sx={{cursor:"pointer"}} onClick={()=>navigate(`${paths.editinvoice}/${invoice._id}`)}>{invoice.vendorDisplayName}</TableCell>
      ...
      <Box sx={{ textAlign: 'center', mt: 2 }}>
        <Typography variant="caption" color="text.secondary">
          {moment(filters.toDate).format('dddd, MMMM DD, YYYY hh:mm A [GMT]Z')}
        </Typography>
      </Box>
    </Box>
  )
