...
const RecievedPayment = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const recivedPaymentData = await apiService.getrecivedPayment(customerId!, { ...paramSearch, type: "bill" });
  ...
  const { mutateAsync: updateInvoicePayments, isPending: isPendingUpdateInvoicePayments } = useMutation({
    mutationFn: async (payload: UpdateRecievedPamentSchemaType) => {
      const response = await apiService.updateRecivedPayment(customerId!, payload, "bill");
