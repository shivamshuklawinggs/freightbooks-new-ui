...
import { TextField, Grid, Select, MenuItem, Checkbox, FormControlLabel, FormControl, InputLabel, } from '@mui/material';
import { useFormContext, Controller } from 'react-hook-form';
import { ICarrier } from '@/types';
import { maxinputAllow } from '@/utils';
import { useDispatch } from 'react-redux';
import { AppDispatch } from '@/redux/store';
import { fetchVendors } from '@/redux/api';
const CompanySection = () => {
  const form = useFormContext<ICarrier>();
  const dispatch = useDispatch<AppDispatch>()
  const [customers, setCustomers] = useState<ICarrier[]>([])

  const getAllCustomers = async () => {
    try {
      const response = await dispatch(fetchVendors({ page: 1, limit: 100 })).unwrap()
      if(response) {
        ...
        form.watch('isSubCustomer') && (
          <Grid item xs={12} md={12}>
            <FormControl fullWidth >
              <InputLabel id="parentCustomer">Parent Vendor</InputLabel>
            <Controller
              name="parentCustomer"
              control={form.control}
              render={({ field }) => (
                ...
                >
                  <MenuItem value="">Select Parent Vendor</MenuItem>
                  {
                   customers.filter((customer: ICarrier) => customer._id !== form.watch('_id')).map((customer: ICarrier,i:number) => (
                    <MenuItem key={customer._id || i} value={customer._id || i}>
