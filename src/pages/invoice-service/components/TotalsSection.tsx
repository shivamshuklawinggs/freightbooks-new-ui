...
const TotalsSection:FC<{handleTaxModalShow:()=>void}> = ({handleTaxModalShow}) => {
 ...
 useQuery({
  ...
  return (
    ...
    {[
      // {
      //   label: getRateInvoice(invoiceType),
      //   value: `$${getRateInvoiceData(invoiceType, form.watch())}`,
      // },
    ...
    {(form.watch('discountPercent') || 0) > 0 && (
      <Box sx={{ mb: 1.5 }}>
        <SelectChartOfAcc removeMasters={["vendor","customer"]} regularExpression="DISCOUNT" type='expense' />
      </Box>
    )}
   ...
   {/* Recieved Payment */}
 
      {
        ...
        >
          <Typography fontWeight={700} color='primary' sx={{display:'flex',alignItems:'center'}} > Payment On {formatDate(payment.paymentDate)} <Edit fontSize='small' onClick={()=>navigate(`${paths.recievedpayment}/${payment.recievedPaymentId}`)}/></Typography>
